...
Comment:
  ...
  - For the second plot, when the null is rejected, the average μ\_hat
    of those data is far away from 0. When the true mean is close to 0,
    the μ\_hat is far away from 0. If the true mean \>= 3, we will
    reject the null for μ\_hat that closes to the true mean because it’s
    considered different from mu = 0.
